...
3. Register a matching `WbToolManifest` via `ctx.wbToolGateway.registerManifest(...)`
   ...
   without a manifest is invisible to `wb-policy` and will be treated as
   maximally restricted by `wb-tool-gateway`'s default-deny fallback — so
   forgetting this step doesn't create a security hole, but it does mean your
   tool will never actually run in the composed system. Test for this.
