...
│   └── presets/                   # one cordis.yml per agent persona (§6.12)
...
- For harness-native tools (`dsh-tool-fs`, `dsh-tool-web`, `dsh-tool-bash`) that
  will never call `registerManifest` themselves, `wb-tool-gateway` ships a
  small `Config`-driven static table mapping their well-known names to a
  manifest, so they are governed too (`Plugin_design_idea`'s capability matrix
  explicitly includes Files/Python/Web/DB for every agent).
...
**Deliverable:** five files under `workbench/cordis/presets/`:

| Preset | Composes (by frozen tool/ctx name, §7) | Capability row (§5-style matrix) |
|---|---|---|
| `document-analyst.cordis.yml` | `wb-rag`, harness `dsh-tool-fs` (read-only), citation-required system prompt | Files ✓ RAG ✓ Vision ○ Python ✗ |
| `engineering-vision.cordis.yml` | `wb-rag`, `wb_vision_analyze`, `wb_ocr_extract`, `dsh-tool-fs` | Files ✓ RAG ✓ Vision ✓ Python ✓ |
| `code-analysis.cordis.yml` | harness `e2b`/`code-runtime`, `dsh-tool-fs`, `wb-rag` (for spec lookup) | Files ✓ RAG ✓ Python ✓ Web ✗ |
| `research.cordis.yml` | harness `dsh-tool-web` (gated PUBLIC-only by `wb-policy`), `wb-rag` | RAG ✓ Web ✓* External API ✗ |
| `artifact.cordis.yml` | `wb-rag`, `wb_generate_report` family | Files ✓ RAG ✓ Artifacts ✓ |
