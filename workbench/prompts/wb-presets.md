...
Produce five `cordis.yml` files under `workbench/cordis/presets/`, per the
table in `DESIGN.md` §6.12:

| File | Composes | Capability shape |
|---|---|---|
| `document-analyst.cordis.yml` | `wb-rag`, harness `dsh-tool-fs` (read-only), citation-required persona | Files ✓ RAG ✓ Vision ○ Python ✗ |
| `engineering-vision.cordis.yml` | `wb-rag`, `wb_vision_analyze`, `wb_ocr_extract`, `dsh-tool-fs` | Files ✓ RAG ✓ Vision ✓ Python ✓ |
| `code-analysis.cordis.yml` | harness `e2b`/`code-runtime`, `dsh-tool-fs`, `wb-rag` (spec lookup) | Files ✓ RAG ✓ Python ✓ Web ✗ |
| `research.cordis.yml` | harness `dsh-tool-web` (gated PUBLIC-only by `wb-policy`), `wb-rag` | RAG ✓ Web ✓* External API ✗ |
| `artifact.cordis.yml` | `wb-rag`, the `wb_generate_*` tool family | Files ✓ RAG ✓ Artifacts ✓ |
