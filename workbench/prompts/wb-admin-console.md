...
- `ctx.wbAudit` (`wb-audit`, a sibling) — fake it, matching
  `WbAuditService`, with fixture `WbAuditEntry` data covering all five
  `kind` values so your dashboard counters and feed have something real to
  render against.
- `ctx.wbPolicy`'s config-override surface (`wb-policy`, another sibling) —
  fake it as described above; document the exact shape you assumed.
