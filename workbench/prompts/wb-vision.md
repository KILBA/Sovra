...
- `ctx.wbModelGateway` (from `wb-model-gateway`, a sibling you won't see
  built) — `resolve(capability): WbModelHandle`. **Fake this for your
  tests**: a small test-only plugin providing `ctx.wbModelGateway` that
  returns a fixed `WbModelHandle` and, critically, a stub for whatever the
  harness's real model-call surface is (`ctx.llm` or similar — see
  `docs/cookbook/adding-an-llm-adapter.md`) that returns deterministic
  canned vision/OCR output. Prefer the real harness tool-registration and
  `Context` machinery around that one faked boundary — per "mock only the
  expensive or non-deterministic boundary (the model), keep everything
  downstream real."
- `ctx.wbToolGateway` (from `wb-tool-gateway`, another sibling) — fake this
  too, matching `WbToolGatewayService` from `wb-types`. Assert your plugin
  actually calls `registerManifest` for both tools; that's testable against
  the fake without needing the real plugin.
...
- Both tools call `ctx.wbModelGateway.resolve(...)` with the correct
  ...
  fake gateway.
- Both tools register a `WbToolManifest` via the faked `wb-tool-gateway`
  exactly once each, at plugin `apply()` time — assert the fake recorded
  both calls with sane `riskLevel`/`dataClassificationCeiling`/
  `networkAccess` values.
